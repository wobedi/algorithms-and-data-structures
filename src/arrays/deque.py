class Deque:
  """Implements https://en.wikipedia.org/wiki/Deque using a dynamic array with a ring buffer"""
  def __init__(self, start_size=10):
    self.arr = [None for i in range(start_size)]
    self.end = 0  # could be any number between [0, start_size)
    self.start = self.end
    self.load = 0
  ...
  def enqueue(self, el):
    ...
    self.load += 1
    print(f'Inserted {el} at index {(self.end - 1 + len(self.arr)) % len(self.arr)}')
    self.check_resize()
  ...
  def resize(self, factor):
    ...
    self.start = 0
    self.end = self.load
    print(f'Resized Deque with factor {factor} to new size {len(self.arr)}')
